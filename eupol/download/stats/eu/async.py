...
def nested_queries_download(
    ...
    with progress:
        call = next(callbacks)
        ini = progress.add_task(description=f"[purple] > ƒ() ⟶ urls from first callback {call.__name__}", total=1)
        urls = call()
        progress.update(ini, advance=1)
        ...
        for i, call in enumerate(callbacks):
            task = progress.add_task(description=f"[purple] >> ⤓ ⤓ get() call on {len(urls)}", total=len(urls))
